# acestream-appimage
Тут зібрані мій bash скрипт, і рецепт для створення AppImage пакета сервера ***AceStream*** в ***pkg2appimage***. Скрипт, за допомогою Docker контейнера на базі docker імеджу debian:10-slim, створює AppImage файл, що містить у собі [`AceStream-3.1.74`](https://download.acestream.media/linux/acestream_3.1.74_debian_10.5_x86_64.tar.gz), взятий на офіційному ресурсі [wiki.acestream.media](https://wiki.acestream.media/Download).

Довгий час основним болем запуску цього сервера була залежність від *python2.7*, який випилений на більшості сучасних десктопних linux операційних системах. Docker контейнер використовую, щоб не встановлювати залежності для роботи ***pkg2appimage*** засмічуючи систему. Не став вимагати можливості роботи скрипта без прав `root`, бо скрипт досить простий, і не складно побачити, що *sudo* використовується тільки для простих операцій з файлами, які створюються в docker mount і власник у них користувач `root`. Для роботи ***pkg2appimage*** в docker контейнері довелося використовувати ключ `--privileged`. Без нього не працював `fuse`. Це не є проблемою безпеки, тому що docker використовуэться тільки для створення пакету appimage. Файл [`pkg2appimage.appimage`](https://github.com/AppImageCommunity/pkg2appimage) взято з офіційного репозиторію на GitHub. Але там кудись подівались попередні версії, а єдина нова - не працює. Наразі використовується версія актуальна на 13 грудня 2021 року.

Передбачена можливість роботи сервера тільки в режимі `--client-console`, але як на мене цього цілком достатньо.
Весь проект займає ***близько 120 мб***, при цьому пакет сервера займає приблизно ***75 мб***. Однак у процесі білда знадобиться щонайменше 600-800 мб
